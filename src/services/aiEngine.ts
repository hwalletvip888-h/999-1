...
function makeSwapCard(input: string, status: CardStatus = "preview"): HWalletCard {
  const lower = input.toLowerCase();
  const amountMatch = input.match(/(\d+(?:\.\d+)?)\s*(u|usdt|美元)?/i);
  const amount = amountMatch?.[1] ?? "100";
  ...
  const rate = wantsBtc ? 78354.8 : 3842.5;
  ...
  return {
    ...
    simulationMode: true,
    userPrompt: input,
    aiSummary: "AI 已为你生成兑换卡片，请确认后再模拟执行。",
    createdAt: new Date().toISOString(),
    title: `${fromSymbol} → ${toSymbol} 兑换`,
    subtitle: "DEX 链上兑换",
    symbol: toSymbol,
    icon: "⇄",
    fromAmount,
    ...
    toAmount,
    toSymbol,
    rate: `1 ${toSymbol} ≈ ${rate.toLocaleString()} USDT`,
    slippage: "0.5%",
    networkFee: "≈ $2.50",
    rows: [
      { label: "支付", value: `${fromAmount} ${fromSymbol}` },
      { label: "获得", value: `${toAmount} ${toSymbol}`, accent: "positive" },
      { label: "汇率", value: `1 ${toSymbol} = ${rate.toLocaleString()} USDT` },
      { label: "滑点", value: "0.5%" },
      { label: "Gas 费", value: "≈ $2.50" }
    ],
    warning: "这是 Mock 卡片，不会真实兑换。",
    primaryAction: "确认兑换",
    secondaryAction: "修改"
  };
}

function makeAgentCard(input: string, status: CardStatus = "running"): HWalletCard {
  const lower = input.toLowerCase();
...
type IntentType = "perpetual" | "swap" | "agent" | "stake" | "unknown";

function detectIntent(text: string): IntentType {
  const lower = text.toLowerCase();
  if (
    lower.includes("做多") || lower.includes("做空") ||
    lower.includes("开仓") || lower.includes("合约") ||
    lower.includes("long") || lower.includes("short") ||
    lower.includes("永续")
  ) return "perpetual";
  if (
    lower.includes("兑换") || lower.includes("swap") ||
    lower.includes("换成") || lower.includes("买入")
  ) return "swap";
  if (
    lower.includes("网格") || lower.includes("grid") ||
    lower.includes("策略") || lower.includes("agent") ||
    lower.includes("自动")
  ) return "agent";
  if (
    lower.includes("质押") || lower.includes("stake") ||
    lower.includes("赚币") || lower.includes("earn") ||
    lower.includes("理财")
  ) return "stake";
  return "unknown";
}

export function generateAIResponse(input: string): ChatMessage[] {
  const intent = detectIntent(input);
  const messages: ChatMessage[] = [];

  switch (intent) {
    case "perpetual": {
      const card = makePerpetualCard(input);
      messages.push({
        id: makeId("msg"),
        role: "assistant",
        kind: "text",
    ...
    case "agent": {
      const card = makeAgentCard(input);
      messages.push({
        id: makeId("msg"),
        role: "assistant",
        kind: "text",
    ...
    case "stake": {
      const card = makeStakeCard(input);
      messages.push({
        id: makeId("msg"),
        role: "assistant",
        kind: "text",
    ...
    default: {
      messages.push({
        ...
        role: "assistant",
        kind: "text",
        text: `收到你的消息："${input}"。我可以帮你执行合约交易、DEX 兑换、网格策略或链上质押。请告诉我你想做什么？`,
        createdAt: new Date().toISOString(),
      });
      break;
    }

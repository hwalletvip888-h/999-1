...
- leverage: 杠杆倍数，默认 10
- toAddress: 转账目标地址（用户提到的链上地址，原样复制）
...
**重要：**
- 用户说「充值」「收款地址」「我的地址」「转入」「存入」→ action 必须是 address
- 用户说「转账」「提现」「发送」「转给」「转出」→ action 必须是 transfer；若提到地址则填 toAddress；若提到 SOL 则 chain=solana，否则 chain=evm
